...
One simple function is `gg_miss_var()`. It creates a lollipop plot (see @sec-dumbbell-and-lollipop-chart). It shows which columns (variables) contain the greatest amount of missing (NA) values.
...
We're also interested in seeing if there is overlapping *missingness* between the columns. It can indicate patterns in the data. We'll use an UpSet plot (see @sec-upset-diagram) for that. Just add `gg_miss_upset()`.
