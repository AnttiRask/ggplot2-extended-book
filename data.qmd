...
Here are some of the reasons why __Movies__ is a good example data set:

- There's a goldilocks amount of data. Not too little, not too much
- There are _categorical_ data of both _nominal_ (title, genre) and _ordinal_ (mpaa) kind
- There are _numerical_ data of both _continuous_ (budget, length, rating) and _discrete_ (year, votes) kind
