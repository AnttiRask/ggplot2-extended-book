...
Now we’re ready for the visualization itself. Besides ggHoriPlot and ggplot2, we’ll be using __ggthemes__ [@arnold2024] to provide us the theme. We'll dive deeper into _themes_ (including ggthemes in [9.1.6 ggthemes](@sec-ggthemes)) later on in [9 Themes](@sec-themes).
...
But let’s first examine the data we’ll be using for this. Time series data works best for this type of chart. Let’s stick to the **ggplot2movies** [@wickham2015] data set that we first encountered in [2.1.2 IMDb movies (1893-2005)](@sec-imdb-movies-1893-2005) and the __movies_na__ tibble.
...
Let's take a look at another ribbon chart. We can use the data set from [3.1.1 Horizon chart](@sec-horizon-chart), which contains average daily temperatures in Helsinki from 2000 to 2024. We'll compare the two years, 2000 and 2024. Which one had more warmer days?
...
If you’ve read the book in chronological order, you’ve already encountered a streamgraph in [Preface](@sec-landscape). We will use that visualization as the example in this chapter.
...
We won’t reiterate the definitions of the different __types__ (see [Preface](@sec-landscape) for them). But we can see that the _count_ for each type starts small in 2007 and increases significantly by 2024.
...
That's why we'll use __showtext__ [@qiu2024] (read more about it in [ 9.6.2 showtext](@sec-showtext)).
...
The labels are in an HTML-style format. To be used later with __ggtext__ [@wilke2022]. We’ll delve into this topic in more detail in [9.4.1 ggtext](@sec-ggtext).
...
We get back to them both in detail later, but colorspace (see more in [16.2.2.1 colorspace](@sec-colorspace)) is used for color manipulation. In this graph, we're making the borders of the areas slightly darker than the areas themselves. ggrepel (see more in [5.10 Repel overlapping labels & text](@sec-repel-overlapping-labels-and-text)) is used for creating labels that don't overlap.
