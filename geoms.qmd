...
For Likert charts, __ggstats__ [@larmarange2025] is my package of choice. It also has other functionalities, which  we’ll return to in @sec-ggstats.
...
By default, `gglikert()` ignores the NA values. If we wanted to, we could turn them into a character string. And then use the __exclude_fill_values__ argument to "[count] them in the denominator for computing proportions" [@larmarange2025].
