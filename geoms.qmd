...
#### Viz #1: Helsinki temperatures, part I - ggHoriPlot {#sec-viz-1-helsinki-temperatures-part-i-gghoriplot}
...
# 7 evenly spaced values across the temperature range, minus the midpoint
# (ggHoriPlot's scale input expects an even number of cutpoints)
scale <- cutpoints %>%
  summarize(
  ...
  # round-trip to tibble so we can use dplyr::slice()
  tibble() %>%
  slice(-4) %>% 
  pull(.)
```
...
ggplot(temperature_hki) +
  ...
  # Diverging palette, reversed so red = warm, blue = cold
  ...
  theme(
    ...
    # No vertical space between yearly facets
    panel.spacing.y = unit(0, "lines"),
    plot.caption    = element_text(size = 8, hjust = 0, color = "#777777"),
    # Left margin bumped up so "Jan" isn't clipped
...
#### Viz #2: IMDb movies, Part I - ggbraid {#sec-viz-2-imdb-movies-part-i-ggbraid}
...
movies_na_wide <- movies_na_long %>%
  ...
  # fill_condition = TRUE when Drama outpaced Comedy that year
...
ggplot() +
  ...
  # Hide the fill legend; keep only the linetype legend for genre
  guides(fill = "none") +
...
#### Viz #3: Helsinki temperatures, part II - ggbraid {#sec-viz-3-helsinki-temperatures-part-ii-ggbraid}
...
temperature_hki_wide <- temperature_hki_long %>%
  ...
  # fill_condition = TRUE when 2024 was warmer than 2000 that day
...
ggplot() +
  ...
  # Blue for 2000 warmer, orange for 2024 warmer
  ...
  # Append °C symbol to y-axis labels
  scale_y_continuous(labels = ~ str_glue("{.x} °C")) +
  
  # Hide the fill legend; keep only the linetype legend for year
  guides(fill = "none") +
  ...
  theme(
    ...
    legend.key.size  = unit(2, "line"),
    legend.position  = c(0.83, 0.12), # relative position inside plot
    legend.text      = element_text(size = 10),
...
#### Viz #4: ggplot2 dependencies (ggstream) {#sec-viz-4-ggplot2-dependencies-ggstream}
...
annotation_numbers <- ggplot2_dependencies_by_year %>%
  ...
  # sort alphabetically so the manual y-positions below line up
  arrange(type) %>%
  mutate(
    # manual y-positions matched to each stream layer's vertical center
    y = c(440, 75, -350),
    # HTML spans for ggtext::geom_richtext() to render with per-type colors
    label = case_when(
      type == "Depends" ~
...
ggplot(ggplot2_dependencies_by_year) +
  ...
  # Anchor point for the first version label
  geom_point(
    aes(x = 2007, y = 0),
    data = tibble(),
    size = 1.5
  ...
  # Version markers along the streamgraph (manually positioned)
  geom_label_repel(
  ...
  geom_stream(
    aes(
      ...
      y     = n,
      fill  = type,
      color = after_scale(darken(fill)) # borders slightly darker than fills
    ),
    # bw controls waviness (bandwidth for kernel density); closer to 1 = less wavy
    bw        = 1,
    linewidth = 0.1
  ) +
...
#### Viz #5: PISA 2022 Questionnaire (gglikert) {#sec-viz-5-pisa-2022-questionnaire-gglikert}
...
gglikert(
  ...
  # Override the default facet layout: move statement labels to the left and wrap them
  facet_grid(
    rows     = vars(.question),
    switch   = "y",
    labeller = label_wrap_gen(30) # wrap long labels at ~30 chars
  ) +
  
  # Diverging red-to-blue palette mirrors the disagree-to-agree axis
  scale_fill_manual(
    values = c(
  ...
  theme(
    ...
    # "top" here means outside the panels — in this left-facet layout, that's the left edge
...
#### Viz #6: IMDb movies, Part II - marimekko {#sec-viz-6-imdb-movies-part-ii-marimekko}
...
movies_marimekko <- movies_na %>%
  ...
  select(mpaa, genre) %>%
  mutate(
    # MPAA levels ordered by restrictiveness, not alphabetically
    mpaa = mpaa %>% factor(levels = c("PG", "PG-13", "R", "NC-17")),
    # fct_infreq() orders genre levels by frequency, largest first
...
plot_marimekko +
  ...
  geom_marimekko_text(
    aes(
      label = after_stat(
        if_else(weight < 70, NA, weight) # hide labels on small tiles
      )
    ),
    color   = "white",
    fontface = "bold",
...
The label code is much simpler than what we'd have needed before. `geom_marimekko_text()` handles many things that I had to calculate manually when using ggmosaic.
...
#### Viz #7: IMDb movies, Part III - ggrain {#sec-viz-7-imdb-movies-part-iii-ggrain}
...
movies_ggrain <- movies_na %>%
  pivot_longer(Action:Short, names_to = "genre") %>%
  filter(
    genre %in% c("Action", "Documentary", "Short"),
    length < 500, # drop unusually long films (outliers)
    value != 0,
    year > 2000
  ) %>%
  select(genre, rating)
...
movies_ggrain %>%
  ...
  geom_rain(
    # cov groups rainclouds by genre while sharing x = 1 in the aes
    cov              = "genre",
    boxplot.args.pos = list(
      ...
      width = 0.1 # boxplot width (separate from the dodge width above)
    )
  )
```
...
# Axis label colors, ordered to match the factor levels after fct_reorder()
colors_vis_7_axis <- c("#3772FF", "#43B929", "#f487b6")

# Summary function for stat_summary: positions sample-size label above each group
add_sample <- function(x) {
  return(
    c(
      y     = max(x) + 0.025, # nudge above the highest point
      label = length(x)       
    )
  )
}
...
ggplot(
  movies_ggrain,
  aes(
    fct_reorder(genre, rating, .desc = TRUE), # order genres by median rating
    rating,
    fill  = genre,
    color = genre
  )
  ...
  geom_rain(
    rain.side    = "r", # density ("rain") on the right
    boxplot.args = list(
      color         = "black",
      outlier.shape = NA # hide outliers — all points are already shown
    ),
    point.args     = list(alpha = 0.1),
    point.args.pos = list(position = position_jitter(seed = 1, width = 0.06))
  ) +
  
  # Median value, printed above each raincloud
  stat_summary(
    aes(
      label = round(after_stat(y), 2),
      # stage() + after_scale() darkens the text relative to each group's color
  ...
  # Sample size, printed beside each raincloud
  stat_summary(
    aes(
  ...
  # Fills are lighter variants of the outlines for contrast
  scale_fill_manual(
  ...
  # Horizontal layout; clip = "off" lets the "n =" labels extend past the panel
...
#### Viz #8: Helsinki temperatures, part III - ggridges {#sec-viz-8-helsinki-temperatures-part-iii-ggridges}
...
temperature_ggridges <- temperature_hki %>%
  filter(year == 2024) %>%
  mutate(
    month_name = factor(
      month.abb[month], # month.abb is base R: c("Jan", "Feb", ..., "Dec")
      levels  = month.abb,
      ordered = TRUE
    ) %>%
      # Reverse so Jan ends up at the top of the ridgeline (y axis is bottom-up)
      fct_rev()
  ) %>%
...
ggplot(
  ...
  aes(
    avg_temperature_celsius,
    month_name,
    fill = after_stat(x) # map fill to the x value (temperature) for a gradient
  ) 
) +
...
Let's first create a Geometric Primitives Visual Catalog. It's a single composite figure showing all ggforce primitives at a glance. We'll use **patchwork** [@pedersen2025c] to combine the multiple plots into one visualization (see @sec-arranging-multiple-plots for more).
...
#### Viz #9: Helsinki temperatures, part IV - ggforce {#sec-viz-9-helsinki-temperatures-part-iv-ggforce}
...
# Monthly averages for 2000 and 2024
temperature_radial <- temperature_hki %>%
  ...
  # Each month occupies 1/12 of the circle (pi/6 radians). We start at the
  # top (12 o'clock = pi/2) and go clockwise, which is decreasing angle in
  # standard math convention. The offset nudges the wedges 2 months
  # counterclockwise so they align with the month labels.
  mutate(
    month_width = 2 * pi / 12,
...
# Inner ring = 2000, outer ring = 2024. The gap between r = 2.0 and r0 = 2.2 
# is the visual separator between rings.
...
# Month labels positioned just outside the outer ring
month_labels <- tibble(
  month     = 1:12,
  label     = month.abb,
  # Point to the CENTER of each wedge — hence (month - 0.5)
...
ggplot() +
  ...
  # Temperature wedges first, so the center disc can cover their inner edges
  geom_arc_bar(
    data = radial_data,
    aes(
      x0    = 0,
  ...
  # White disc covers the inner hole created by r0 = 1.0 on the first ring
  geom_circle(
    aes(x0 = 0, y0 = 0, r = 0.9),
    fill  = "white",
    color = NA
  ...
  # Center text, year labels, and month labels
  annotate(
    "text",
    x        = 0,
    y        = 0.15,
  ...
  # Month labels stay horizontal — no rotation around the circle
  geom_text(
    data = month_labels,
...
However, ggdiagram operates outside the ggplot2 paradigm and only uses it to plot the results, so we won't go further into the package here. If you are interested in learning more, check out the package's [documentation](https://wjschne.github.io/ggdiagram/).
...
#### Viz #10: Helsinki temperatures, part V - ggTimeSeries {#sec-viz-10-helsinki-temperatures-part-v-ggtimeseries}
...
There is a fun **Little Miss Data** [blog post](https://www.littlemissdata.com/blog/set-analysis) [@ellis2019] that pits the two against each other. But as you’ll find out by reading the blog (spoilers), the two are for very different purposes.
...
#### Viz #11: IMDb movies, Part IV - ggupset {#sec-viz-11-imdb-movies-part-iv-ggupset}
...
ggplot(movies_ggupset, aes(genres)) +
...
#### Viz #12: IMDb movies, Part V - ggVennDiagram {#sec-viz-12-imdb-movies-part-v-ggVennDiagram}

For this, let's revisit the IMDb movies data set. We'll first limit the number of sets to three, **Comedy**, **Drama**, and **Romance**. What we want for ggVennDiagram is a list that has three levels, one for each genre.
...
movies_ggvenndiagram <- movies_na %>%
...
genre_sets <- genres %>%
  set_names() %>%
  map(~ movies_ggvenndiagram$title[movies_ggvenndiagram[[.x]] == 1])
```
...
movies_ggvenndiagram_all <- movies_na %>%
  filter(between(year, 1900, 2000)) %>%
...
genre_sets_all <- genres_all %>%
  set_names() %>%
  map(~ movies_ggvenndiagram_all$title[movies_ggvenndiagram_all[[.x]] == 1])
```
...
ggVennDiagram(
  ...
  # Extra horizontal padding so the outer set labels don't get clipped
...
Line charts are the bread and butter of data visualization. And as with bar charts, ggplot2 handles most types out of the box. And as with bar charts, there are specific use cases for these more specialized line charts.

In this section, we’ll take a look at the **bump chart**, the **dumbbell and lollipop chart**, the **line chart with neon glow or shadow effects**, and the **slope chart**.

### Bump chart {#sec-bump-chart}

A bump chart is a type of line chart used to visualize changes in rankings of categorical data over time. To create one, we'll use the extension **ggbump** [@sjoberg2026].

We'll once again use the IMDb movie dataset. This time, we'll summarize the data by genre and decade, starting from the 1930s. Before that, some genres have only a handful of films, making average ratings unreliable.

#### Viz #13: IMDb movies, Part VI - ggbump {#sec-viz-13-imdb-movies-part-vi-ggbump}
...
movies_ggbump <- movies_na %>%
  pivot_longer(Action:Short, names_to = "genre") %>%
  filter(value != 0) %>%
  filter(decade %in% seq(1930, 2000, 10)) %>%
  summarize(
    rating_avg = mean(rating),
    .by        = c(genre, decade)
  ) %>%
  mutate(
    rank = rank(-rating_avg, ties.method = "first"),
    .by  = decade
  ) %>%
  arrange(decade, rank)
```
...
Let's start with the basic version with default settings.
...
movies_ggbump %>%
  ggplot(aes(decade, rank, color = genre)) +
  geom_bump()
```

We can already see what the chart is about. The movie genres are in the ranking order, but the order is upside down. Next, let's fix that and make other changes to make the chart more presentable.

We'll add the points first. They act as anchors for the viewer's gaze. Then we'll add the genres using `geom_text()`. Direct labeling is a nice touch and more intuitive than using a legend.

In this book, color as a storytelling tool is a recurring theme. And it was a conscious storytelling decision to highlight the Documentaries genre. We can use `scale_color_manual()` to specify only one genre's color, while ggplot2 uses its default palette for the rest.

Finally, we need to fix the ranking order using `scale_y_reverse()`.
...
movies_ggbump %>%
  ...
  geom_text(
    ...
    data     = movies_ggbump %>% filter(decade == min(decade)),
    size     = 4,
    hjust    = 1,
    fontface = "bold"
  ) +
  ...
  geom_text(
    ...
    data     = movies_ggbump %>% filter(decade == max(decade)),
    size     = 4,
    hjust    = 0,
    fontface = "bold"
  ) +
  ...
  geom_bump(
    linewidth = 1.2,
    smooth    = 8 # higher = smoother curves
  ) +
  
  scale_color_manual(values = c("Documentary" = "#CC4C00")) +
  
  scale_x_continuous(
    breaks = seq(1930, 2000, 10),
    # Extra horizontal padding on both sides for the direct labels
  ...
  labs(
    title    = "Documentaries Are the Best",
    subtitle = "Genres ranked by their average IMDb rating",
    caption  = "Data: IMDb movies (1893-2005) via ggplot2movies | Visualization: Antti Rask",
    x        = NULL,
    y        = NULL
  ) +
...
We made some bold choices, but now we have a chart that tells one of the stories buried within the data. Bump charts work best when you have a handful of categories competing across discrete time steps, and you care about relative position rather than absolute values.

The ranking abstraction strips away the noise and lets the audience focus on who's rising and who's falling. And in this case, the answer is clear.

### Dumbbell and lollipop chart {#sec-dumbbell-and-lollipop-chart}

Bar charts do a lot of heavy lifting, but aren't always the right tool. When all you need is to mark a single value per category, a full bar can feel like a lot of ink for one number. That's where the *lollipop chart* comes in, swapping the bar for a thin stem and a dot.

And when you want to show how a value *changed* between two states, before and after, then and now, the *dumbbell chart* draws both points and connects them, so the gap itself becomes the message.

**ggalt** [@rudis2026] gives us `geom_lollipop()` and `geom_dumbbell()` for exactly these jobs. We'll build up in three steps, all on the Helsinki temperature data: a plain lollipop of 2024's monthly means, the same months measured as anomalies against the previous two decades, and finally a dumbbell comparing two years twenty years apart.

#### Viz #14: Helsinki temperatures, part VI - ggalt {#sec-viz-14-helsinki-temperatures-part-vi-ggalt}

The lollipops use 2024, but the second one needs something to compare against. We first build a per-month baseline from every prior year on record (2000–2023), then average 2024 by month and attach that baseline, so the **anomaly** column holds each month's departure from its own historical normal. Both lollipop charts draw from this one tibble. The first uses **temp_avg**, the second, anomaly.
...
temperature_baseline <- temperature_hki %>%
  filter(year < 2024) %>%
  summarize(
    temp_base = mean(avg_temperature_celsius),
    .by       = month
  )

temperature_lollipop <- temperature_hki %>%
  filter(year == 2024) %>%
  summarize(
    temp_avg = mean(avg_temperature_celsius),
    .by      = month
  ) %>%
  left_join(temperature_baseline, by = "month") %>%
  mutate(
    temp_anomaly = temp_avg - temp_base,
...
temperature_lollipop %>%
  paged_table()
```

As we can see, BLAH BLAH BLAH...

Let's take a look at the first two charts, BLAH BLAH BLAH...

Let's start with the simplest thing the `geom_lollipop()`can do: one dot per month, placed at its average temperature, with a stem dropping to zero. Because we're plotting actual temperatures, that baseline is literally freezing. The winter months hang below it while summer reaches up toward twenty.
...
ggplot(temperature_lollipop, aes(month, temp_avg, color = temp_avg >= 0)) +
  ...
  labs(
    title    = "Average monthly temperature in Helsinki for 2024",
    subtitle = "A lollipop marks each month's mean",
    x        = NULL,
    y        = NULL,
    caption  = "Data: Finnish Meteorological Institute open data, weather observations (CC BY 4.0) | Visualization: Antti Rask"
  ) +
...
Now the more interesting question: not how warm each month *was*, but how warm it was *compared with the past*. Subtracting each month's 2000-2023 average turns the chart into a record of where 2024 ran warmer or cooler than its own history. Zero is the long-term normal.
...
ggplot(
  temperature_lollipop,
  aes(month, temp_anomaly)) +
  geom_hline(yintercept = 0) +
  geom_lollipop()
```

And with the finalized version, red months beat the long-term normal, blue months fell short. As before, leaving `point.color` unset lets the dot inherit the color mapped in `aes()`, so stem and dot move together.

```{r}
temperature_lollipop %>%
  ggplot(aes(month, temp_anomaly, color = temp_anomaly >= 0)) +
  geom_hline(
    yintercept = 0,
    color     = "grey60",
    linewidth  = 0.25
  ...
  labs(
    title    = "Most months in 2024 were warmer than the same months during the previous two decades",
    subtitle = "Each month against its 2000–2023 average",
    x        = NULL,
    y        = NULL,
    caption  = "Data: Finnish Meteorological Institute open data, weather observations (CC BY 4.0) | Visualization: Antti Rask"
  ) +
...
The lollipop shows one moment; the dumbbell shows movement. Each month now gets two dots: its 2004 mean and its 2024 mean, joined by a segment whose length *is* the change.

To get the right data, we pivot to a wide shape, one column per year, keeping the months in calendar order.
...
temperature_dumbbell <- temperature_hki %>%
  ...
  pivot_wider(
    names_from   = "year",
    names_prefix = "temp_avg_",
    values_from  = "temp_avg"
  ) %>%
  mutate(
    month = factor(month.abb[month], levels = month.abb)
  )
...
#| results: asis
#| 
temperature_dumbbell %>%
  paged_table()
```

We keep the months in calendar order rather than reordering by value: with a lollipop a ranking can illuminate, but a dumbbell's story is the gap, and calendar order lets the eye scan the seasons and notice which months have the widest gap.
...
ggplot(
  ...
  geom_dumbbell() +
  geom_point(aes(x = temp_avg_2004, color = "2004"), size = 3) +
  geom_point(aes(x = temp_avg_2024, color = "2024"), size = 3) +
  labs(
    color = NULL,
    x     = NULL
  )
...
The two years live in the subtitle as colored text via ggtext, which means we can keep the plot itself free of a legend.
...
ggplot(
  ...
  labs(
    title    = "Most of the average monthly temperatures in Helsinki have risen in 20 years",
    subtitle = "From <span style = 'color:#9FD4D6;'>**2004**</span> to <span style = 'color:#0F6E70;'>**2024**</span>",
    caption  = "Data: Finnish Meteorological Institute open data, weather observations (CC BY 4.0) | Visualization: Antti Rask",
    x        = NULL,
    y        = NULL
  ) +
...
Both geoms are conveniences worth seeing through: a lollipop is a `geom_segment()` to zero plus a `geom_point()`, and a dumbbell is a segment between two values with a dot at each end. You could build either from base ggplot2 if you'd rather not take the dependency.
