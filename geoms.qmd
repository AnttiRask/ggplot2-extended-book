...
To create a mosaic chart, we’ll be using **marimekko** [@kałędkowski2026]. It provides `geom_marimekko()` as a native ggplot2 layer, so you can combine it with any other ggplot2 functionality (facets, themes, annotations, etc.). You can use it to create spine plots, stacked bar charts, mosaic charts, and multi-variable mosaics.

**Note!** The first version of this chapter was written with **ggmosaic** [@jeppson2021] in mind. But after it was archived from **CRAN** on 2025-11-10 (as issues were not corrected despite reminders) and even the version on GitHub isn't working currently, I was forced to find a replacement. And marimekko is that package.
...
The key difference from the old ggmosaic approach is that marimekko uses a formula to specify the variables instead of wrapping them in `product()`. The formula `~ mpaa | genre` tells marimekko to use mpaa for the columns and genre for the segments within each column.
...
First, let's take a look at those text labels. marimekko comes with a `geom_marimekko_text()` function that reads tile positions automatically from the preceding `geom_marimekko()` layer. You only need to provide the **label** aesthetic. This is much simpler than the old approach, where you had to extract tile coordinates manually from the built plot object.

We can reference computed variables via `after_stat()`. The computed variable **weight** gives us the aggregated count for each tile. Some of the tiles are so small it doesn't make sense to add labels to them. We'll use `after_stat()` to conditionally hide labels when the count is under 70.
...
The label code is much simpler compared to what we'd have needed before. With marimekko, we don't have to extract tile data from the built plot, calculate center positions, count the combinations, and join everything together. Instead, `geom_marimekko_text()` handles all of that internally.
